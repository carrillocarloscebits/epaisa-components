    ...
    >
      ...
      <TouchableOpacity
        style={{
          ...
          //here
          alignItems: 'center',
          justifyContent: isTablet ? custom ? 'space-around' : 'flex-start' : 'flex-start',
          paddingHorizontal: isTablet ? custom ? wp('0%') : 0 : 0
        }}
        disabled={disabled}
        onPress={() => openModal()}
      >
        <Image
          style={{ width: isTablet ? hp('7') : wp('7.4'), height: hp('4%'), marginRight: isTablet ? custom ? 0 : wp('2%') : wp('2%') }}
